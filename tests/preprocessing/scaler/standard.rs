...
mod basic {
    ...
    fn test_fit_transform() {
        ...
        for j in 0..2 {
            ...
            let std: f64 = col.std(0.0);
            approx_eq(mean, 0.0, 1e-10);
            approx_eq(std, 1.0, 1e-10);
        }
    }
    ...
    fn test_fit_simple_data() {
        ...
        assert_vec_approx_eq(&result.column(0).to_vec(), &expected, 1e-6);
    }
...
mod partial_fit {
    ...
    fn test_partial_fit_incremental() {
        ...
        let all_data = make_matrix(4, 2, &[1.0, 2.0, 2.0, 4.0, 3.0, 6.0, 4.0, 8.0]);
        let scaler_batch = StandardScaler::new();
        let fitted_batch = scaler_batch.fit(all_data.view()).unwrap();
        let scaler_incr = StandardScaler::new();
...
mod trait_impl {
    ...
    fn test_inversible_transformer_trait() {
        ...
        let scaler = StandardScaler::new();
        let fitted = scaler.fit(x.view()).unwrap();
        let transformed = <StandardScaler<Fitted> as Transformer>::transform(&fitted, x.view()).unwrap();
        let recovered = <StandardScaler<Fitted> as InversibleTransformer>::inverse_transform(&fitted, transformed.view()).unwrap();
        assert!(matrix_all_close(&x, &recovered, 1e-10));
    }
